...
% FORMAT rcp('Batch_Fit',ID,tau,dt,P,T,Td)
% ID              : ID of model
...
% P               : Phase state of the desired dmp
...
%     rcps(ID).psi = exp(-(mod(rcps(ID).p,2*pi)*ones(1,length(rcps(ID).c))-rcps(ID).c).^2.*rcps(ID).D');
...
%     rcps(ID).pd = pi/tau;
    rcps(ID).pd = pi/tau; %%% It should be independent of tau
...
% .........................................................................
  case 'run_fit'
    ...
    % the weighted sum of the locally weighted regression models
    rcps(ID).psi = exp(rcps(ID).D.*(cos(rcps(ID).p-rcps(ID).c)-1));
          PSI = exp(-(mod(P,2*pi)*ones(1,length(rcps(ID).c))-ones(length(T),1)*rcps(ID).c').^2.*(ones(length(T),1)*rcps(ID).D'));
...
%     end
    ...
    % the regression target
    amp = A;
    Ft  = (Tdd*tau^2-rcps(ID).alpha_z*(rcps(ID).beta_z*(ym-T)-Td*tau));
...
%     PSI = exp((cos(P*ones(1,length(rcps(ID).c))-ones(length(T),1)*rcps(ID).c')-1).*(ones(length(T),1)*rcps(ID).D'));
    ...
    rcps(ID).w    = rcps(ID).sptd./(rcps(ID).sp2+1.e-15);
